...
document.body.appendChild(buatTombol('history','blue', '320px'))
...
function buatTombol(id, text, warna){
    ...
    tbl.setAttribute('style','margin: 0 5px; border-radius; 5px; border : none; padding: 10px; cursor; pointer; background-color; #45919d; color; white; font-weight; bold;')
    tbl.innerHTML = text;
    return tbl
}
...
tambah.addEventListener("click", function () {
    ...
    if (x == "" || y == "") {
        document.getElementById("hasil").innerHTML = "Inputan harus diisi";
    } 
    else if (isNaN(x) || isNaN(y)) {
        document.getElementById("hasil").innerHTML = "Inputan harus berupa angka";
    } 
    else {
        let hasil = hitung('+', Number(x), Number(y));
